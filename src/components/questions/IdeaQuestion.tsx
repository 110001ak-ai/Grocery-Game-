...
export default function IdeaQuestion({ textValue, onTextChange, onSelect }: Props) {
  ...
  const vibrate = (p: number | number[]) => { if (navigator.vibrate) navigator.vibrate(p); };
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      setPV(false);
      setTimeout(() => { setIdx(i => (i + 1) % PLACEHOLDERS.length); setPV(true); }, 400);
    }, 3000);
  ...
  const toggleMic = useCallback(() => {
    ...
    recognition.onstart  = () => { vibrate(60); setMicState("listening"); setInterim(""); };
    ...
    recognition.onerror = () => { vibrate([30, 50, 30]); setMicState("idle"); setInterim(""); };
    recognition.onend   = () => { setMicState("idle"); setInterim(""); };
    recognition.start();
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="relative">
        ...
        {isListening && (
          <div className="absolute inset-0 rounded-2xl pointer-events-none z-10 overflow-hidden">
            ...
            {/* Pink shimmer sweep */}
            <div className="absolute inset-0" style={{
              background: "linear-gradient(90deg,transparent 0%,rgba(255,45,110,0.06) 50%,transparent 100%)",
              backgroundSize: "200% 100%",
              animation: "ideasShimmer 2s linear infinite",
            }} />
            {/* Existing text rendered cleanly on top */}
            <div className="absolute inset-0 px-5 py-5 overflow-hidden" style={{
              fontSize: "14px",
              fontWeight: 600,
              lineHeight: "1.625",
              ...
              {textValue && (
                <span style={{ color: "rgba(255,255,255,0.85)" }}>{textValue}</span>
              )}
            ...
            {/* Empty state */}
            {!textValue && !interim && (
              <div className="absolute top-5 left-5 right-16 pointer-events-none" style={{
                fontSize: "14px", fontWeight: 600, color: "rgba(255,45,110,0.5)",
        ...
        <textarea
          ...
          style={{
            ...
            border:       `2px solid ${
              isListening  ? "#ff2d6e"
              : justStopped ? "#00e5c3"
              : focused    ? "rgba(232,104,10,0.5)"
              : hasIdea    ? "rgba(0,229,195,0.35)"
              : "var(--border)"
            }`,
        ...
        {!textValue && !isListening && !focused && (
          <div className="absolute top-5 left-5 right-16 pointer-events-none font-semibold leading-relaxed"
            style={{
              fontSize:  "14px",
              color:     "var(--text3)",
        ...
        {/* Progress bar */}
        {charCount > 0 && (
          <div className="absolute bottom-0 left-0 right-0 h-[3px] rounded-b-2xl overflow-hidden" style={{ zIndex:20 }}>
            <div style={{
              height:     "100%",
            ...
            <button
              ...
              style={{
                ...
                background: isListening
                  ...
                  : "var(--opt-bg-h)",
                border: `1.5px solid ${
                  isListening  ? "rgba(255,45,110,0.7)"
                  : justStopped ? "rgba(0,229,195,0.7)"
                  : "var(--border)"
                }`,
                boxShadow: isListening
      ...
      {isListening && (
        <div className="relative flex items-center gap-3 px-4 py-3 rounded-2xl overflow-hidden"
          ...
          <div className="absolute inset-0 pointer-events-none" style={{
            background:     "linear-gradient(90deg,transparent,rgba(255,45,110,0.07),transparent)",
            backgroundSize: "200% 100%",
            animation:      "ideasShimmer 2s linear infinite",
          }} />
          ...
          <div className="relative flex-shrink-0" style={{ width:34, height:34 }}>
            ...
            <div className="absolute inset-0 rounded-full flex items-center justify-center"
              style={{ background:"rgba(255,45,110,0.15)", animation:"ideasBreathe 1.5s ease-in-out infinite" }}>
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none"
                stroke="#ff2d6e" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                <rect x="9" y="2" width="6" height="12" rx="3"/>
                <path d="M5 10a7 7 0 0 0 14 0"/>
                <line x1="12" y1="19" x2="12" y2="22"/>
                <line x1="8"  y1="22" x2="16" y2="22"/>
          ...
          {/* Waveform */}
          <div className="flex items-end gap-[3px] flex-shrink-0 relative z-10" style={{ height:20 }}>
            {[0,1,2,3,4,5,6].map(i => (
              <div key={i} style={{
                width:          3,
                height:         20,
      ...
      {justStopped && (
        <div className="flex items-center gap-2.5 px-4 py-2.5 rounded-2xl"
          ...
          <span className="font-extrabold text-[12px]" style={{ color:"#00e5c3" }}>
      ...
      <div className="flex items-center justify-between px-1">
        <span className="text-[11px] font-semibold" style={{ color:"var(--text3)" }}>
          {isListening       ? "🎙️ Listening..."
           : justStopped     ? "✅ Recording saved"
           : charCount === 0 ? "No wrong answers here 🎯"
           : charCount < 30  ? "Keep going… 💭"
           : charCount < 80  ? "Now we're talking 🚀"
           : "That's a real take 🔥"}
        </span>
        <div className="flex items-center gap-1.5">
          {charCount > 0 && (
            <span className="text-[10px] font-bold" style={{ color:"var(--text3)" }}>{charCount}</span>
          )}
          ...
          {hasIdea && (
            <span className="text-[10px] font-extrabold px-2 py-0.5 rounded-lg" style={{
              background:"rgba(0,229,195,0.1)", border:"1.5px solid rgba(0,229,195,0.3)", color:"#00e5c3",
